...
class ConveyorCnnTrainer():
    def __init__(self, args):
        ...
        self._dir_path = self._dir_path + '/' if (self._dir_path.find('/') != -1) else self._dir_path
        self._train_data_path = self._dir_path + 'data/training'
        self._test_data_path = self._dir_path + 'data/test'
        self._weights_path = self._dir_path + 'weights/task_' + str(self._args.task) + '_best.pt'
    ...
    def train(self):
        ...
        for epoch in range(1, self._args.epochs + 1):
            ...
            if validation_metric_value > best_validation:
                ...
                torch.save(self._model.state_dict(), self._dir_path + 'weights/task_' + str(self._args.task) + '_best.pt')
            else:
    ...
    def _train_batch(self, task, model, criterion, metric, optimizer, image, segmentation_target, boxes, class_labels):
        ...
        :param metric: La métrique créée in create_metric
        ...
        :param class_labels: Le tenseur cible pour la tâche de classification
            Dimensions : (N, 3)
                ...
                Si un 0 est présent à (i, 0), aucun cercle n'est pas présent dans l'image i.
                Si un 1 est présent à (i, 1), un triangle est présent dans l'image i.
                Si un 0 est présent à (i, 1), aucun triangle n'est pas présent dans l'image i.
                Si un 1 est présent à (i, 2), une croix est présente dans l'image i.
                Si un 0 est présent à (i, 2), aucune croix n'est pas présente dans l'image i.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_cpu', type=bool, default=True, help='use the cpu instead of the gpu (default: True)')
