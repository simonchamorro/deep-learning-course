...
if __name__ =="__main__":
    ...
    test_tagging = True         # Visualiser l'annotation sur des echantillons de validation
    ...
    if test_tagging:
        ...
        for num in range(10):
            ...
            in_seq = input_sequence.unsqueeze(0).unsqueeze(-1)
            out, hn = model(in_seq)
            out = out.squeeze(0).squeeze(-1)
            prediction_sequence = out.detach().numpy()
    ...
    if test_generation:
        ...
        for num in range(10):
            ...
            in_seq = input_sequence[:usable_input_sequence_len].unsqueeze(0).unsqueeze(-1)
            out, hn = model(in_seq)
            out = out[:,-1:,:]
            prediction_sequence[0] = out.detach().numpy()[0][0][0]
            for i in range(nb_predictions_to_generate-1):
                in_seq = out
                out, hn = model(in_seq, hn)
                prediction_sequence[i+1] = out.detach().numpy()[0][0][0]
