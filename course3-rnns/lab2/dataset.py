...
class Fr_En(Dataset):
    ...
    def __init__(self, filename='fra.txt', n_samp=2000,start=0, samplelen=[7,10]):
        ...
        # Fr
        for k, sentence in data['fr'].items():
            sentence.append(stop_symbol)
            if len(sentence) > self.max_len['fr']:
                self.max_len['fr'] = len(sentence) 

        for k, sentence in data['fr'].items():
            pad_len = self.max_len['fr']-len(sentence)
            sentence.extend([pad_symbol]*pad_len)

        # En
        for k, sentence in data['en'].items():
            sentence.append(stop_symbol)
            if len(sentence) > self.max_len['en']:
                self.max_len['en'] = len(sentence) 

        for k, sentence in data['en'].items():
            pad_len = self.max_len['en']-len(sentence)
            sentence.extend([pad_symbol]*pad_len)
